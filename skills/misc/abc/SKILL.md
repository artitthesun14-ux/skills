...
The page opens on the **Layer Map**, drawn as a tree for a reader who does not yet know the industry: the end user on top, and each row below is what the row above needs. Pressing a box shows that layer's card beside the tree. Each card names the layer, says in one plain Thai line what the layer makes or does (`product`: "transmission lines, substations and the interconnection queue", never a finding), and says who trades there: `sellers`, the companies that make or sell what the layer makes, and `customers`, who buys it, named as companies where known (a demand layer carries customers only). Keep the two lists apart, since a layer's company list often mixes in its buyers, and write each customer so the next layer up reads as the seller list of the one below where that is true. A layer with neither falls back to its companies. A layer with sellers also lists `sells_to`: the ids of the layers where its customers appear, read off those lists. Beside it, `buyers` maps each of those ids except `"end"` to the entries of `customers` that sit in that layer, copied word for word, so the card's "ขายให้" row names who buys along each line; the card's customer row then keeps only the customers on no line. The map ends on the **end consumer**, `chain.end`: who finally uses what the chain delivers (`label`, `product`, `who`, `icon`), a plain card after the last layer with no panel and no weight. A layer whose customers sell on to those people, or are those people, lists `"end"` in `sells_to`; any other customer outside every layer adds nothing. The tree comes from `chain.tree`: `title` (the question the tree answers, "บริการ AI ต้องใช้อะไรบ้าง"), `rows` (lists of node ids, row 0 `["end"]`, each later row the layers that sell to the row above), `labels` (a short Thai box name per id, a word or two), and `context` for any node kept off the tree (`label`, `note`), shown as a strip above it; the trend and any layer that sells to nothing on the map go there. Every `sells_to` line of a tree box must reach the row above or a neighbour in the same row, where it is drawn as a sideways arrow; a layer with buyers in two boxes hangs from both, drawn once. When a line cannot fit, fix the data first: a layer whose named customers sit in another layer sells to that layer, not straight to `"end"`. Goods run up the lines and money runs down them. The reader can zoom the tree, and each box shows more as it grows: zoomed out, its step, its `labels` name, its bottleneck role in words and a "มีทางเลือก" mark when it has an alternative pressure; zoomed in, its `product` and its `companies_short` with tickers, two lines each, and in money mode the `flows` label of each payment into the box, written on the line above it. These come from fields the layer already has, so keep `product` and each `flows` label short enough to read in two lines. A hardware layer carries a small line drawing chosen by `icon` from the set the shell holds (chip, memory, wafer, server, tower, turbine, transformer, plant, generator, pipeline, mine, cylinder, centrifuge, fuelrod, rocket, satellite, dish, device); a software, market or policy layer carries none. The map keeps one encoding only, the bottleneck, set by `role`: `both` for the layer that gates and captures, `gate` for one that gates without capturing, `capture` for one that captures without gating, absent for the rest; those cards get the red frame. The weight colours and evidence tags live in the detail panel, so the map stays readable at a glance. Each competing pressure in `chain.pressures` names the node it pushes on as `target` and shows folded shut with that layer's card: its label and effect on top, its lines and companies inside. Each bottleneck carries the step 5 answer as a pressure with `kind: "alternative"`: `label` ("ถ้ากังหันก๊าซติด"), `verdict` (one line on how good the way around is), `tag` (its evidence), and `options`, each with `label`, `when` ("ตอนนี้" or when it arrives), `note` (what it still depends on), and `via` when the option is itself a layer on the map, so the reader can jump to that card. It sits with the bottleneck's card above any other pressure. A buyer's way around belongs here and a force from outside the chain stays a plain pressure, so each idea has one box. Money runs opposite to goods: `chain.flows` gives, for every `sells_to` link, the payment going back as `from` (the buyer, who pays), `to` (the seller) and `label` (what the payment is for, in plain Thai: "ซื้อกังหันเพื่อสร้างโรงไฟใหม่"). Direction stays in `sells_to`; `flows` carries only the label, and every link needs one, including links into `"end"`, paid `from` `"end"`. Where little or no money moves (volunteer work, a free model), the label says so. The reader switches the tree between goods lines and money lines. Under the map, `chain.map_summary` gives four short lines in plain Thai, each `label` and `text`: ใครขายให้ใคร, เงินไหลอย่างไร, คอขวดและทางอ้อม, and ข้อสังเกต (the one thing the map shows that a reader would miss, such as a layer paid whichever way the buyer turns). Write them from the map's own data, one or two sentences each. Clicking a card opens the detail panel.
