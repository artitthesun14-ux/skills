...
## 9. เฟส 4: System Overhaul [P4-A..D, implement ครบแล้ว ดูข้อยกเว้นที่ 9.8]

> ต่อยอดจาก `docs/ux-outfit-color-matcher.md` §1-11 (แท็ก [P4-A..D]) และ `.scratch/outfit-color-matcher/spec.md`
> §1-8 ข้างบนคือระบบภาพของเฟส 1-3 ส่วนนี้คือระบบภาพที่ **shipped จริงตอนนี้** (เฟส 4 ครบทุกเฟส)
...
| Token | ค่า | ใช้ทำอะไร |
|---|---|---|
| `--collage-overlap-max` | `.2` = 20% ของความกว้าง tile (ดู 9.8 ข้อ 1) | เพดานพื้นที่ที่ทรงหนึ่งบังทรงถัดไปได้ กันไม่ให้บังจนดูรก/สับสนว่าทรงไหนเป็นของหมวดไหน |
...
### 9.8 สิ่งที่ต่างจากแบบตอน implement จริง (เฟส C/D, บันทึกไว้ไม่ให้เอกสารโกหก)

ห้าจุดที่โค้ดจริงไม่ตรงกับ 9.1-9.4 ข้างบน พร้อมเหตุผล เอกสารนี้ยึดของจริงเป็นหลัก:

1. **`--collage-overlap-max` เป็นเศษส่วนไม่มีหน่วย (`.2`, จอแคบ `.14`) ไม่ใช่ `20%`** ค่ายังหมายถึง 20%
   ของความกว้างทรงเหมือนเดิม แต่ CSS `calc()` คูณ `%` กับความยาวไม่ได้ ถ้าเก็บเป็น `%` จะคำนวณ
   ความกว้างของทรงและระยะซ้อนทับจากโทเคนตัวเดียวกันไม่ได้เลย
2. **`CollageLegend` เป็นคอลัมน์เดียวทุกขนาดจอ** (9.4 เขียนไว้ว่า 2 คอลัมน์ที่ md/lg) เพราะการ์ดกว้างสุด
   แค่ 480px พอแบ่งสองคอลัมน์แล้วชื่อชิ้นถูกตัดด้วย ellipsis ("กางเกง/กระ…") ซึ่งขัดกฎ "ทุกชิ้นต้องมี
   ชื่อ + hex อ่านได้ครบ" โดยตรง ยืนยันจากสกรีนช็อตรอบแรกก่อนแก้
3. **พื้นหลังไดนามิกสลับด้วย attribute `data-tint` บน `<html>`** ไม่ใช่ให้ `--bg` เป็น `hsl(...)` ตลอดเวลา
   เพราะต้องรับประกันว่า *ก่อน* มีชุดแรก พื้นหลังยังเป็น `#FBFAF7` เดิมเป๊ะตามที่ 9.1 กำหนด
   (`hsl(40 6% 95%)` ไม่เท่ากับ `#FBFAF7` ซึ่งอยู่ที่ราว `hsl(45 33% 98%)`)
4. **`ColorToneModal` ใช้ `<dialog>` ของเบราว์เซอร์จริง** ไม่ได้เขียน overlay เอง: `showModal()` ให้
   focus trap, Escape และการคืนโฟกัสไปปุ่มที่เปิดมันครบตามที่ 9.3 เรียกร้อง โดยไม่ต้องเขียนกลไกใหม่
   ให้พลาดเอง (แตะ scrim ปิด เป็นโค้ดที่เพิ่มเองจุดเดียว)
5. **`ToneGroupDetail` เป็นรายการคอลัมน์เดียว ไม่ใช่กริดสวอตช์** (9.3 เขียนไว้ว่ากริด `ColorSwatch(md)`)
   ด้วยเหตุผลเดียวกับข้อ 2: ทุกเฉดต้องมีชื่อสีกับ hex เป็นข้อความข้างสวอตช์ กริดของสวอตช์ล้วนจะกลายเป็น
   การสื่อด้วยสีอย่างเดียว ส่วน `ToneGroupList` ยังเป็นกริดตามแบบ และชิปกลุ่มมีจุดสีตัวอย่าง 6 จุด
   (`aria-hidden`) ช่วยให้กวาดตาหากลุ่มได้เร็ว โดยชื่อกลุ่มที่เป็นข้อความยังเป็นตัวสื่อจริง
6. **ป้ายของปุ่มเปิดมุมมองโทนไม่ใช่คำว่า "ดูสีเพิ่มเติม" ตามที่ spec.md เขียนไว้** เพราะไม่มีทางเข้าไหน
   ที่ "เพิ่ม" สีจริงๆ เลย: ทางเข้าจากแถวสีของวันนี้แสดง *น้อยกว่า* แถว (เฉพาะสีในตู้ ตาม Decision #4
   ส่วนแถวมีพรีเซ็ตด้วย) ส่วนทางเข้าจากฟอร์มแสดง *ชุดเดียวกัน* กับที่แถวมีอยู่แล้ว แค่จัดกลุ่มให้
   ป้ายจริงจึงเป็น "ดูสีในตู้ตามโทน" และ "ดูสีทั้งหมดตามโทน" ซึ่งบอกสิ่งที่จะเกิดขึ้นตรงตามจริง
7. **ชื่อกลุ่มโทนถูกตรึงเป็นลำดับคงที่ 10 กลุ่ม** (`ขาว-เทา-ดำ`, `เบจ-น้ำตาล`, `แดง`, `ส้ม`, `เหลือง`,
   `เขียว`, `ฟ้า`, `น้ำเงิน`, `ม่วง`, `ชมพู`) ทุกคำมาจากคลังคำของ `colorNameTH()` เดิม ไม่ใช่ taxonomy ใหม่
   และ **เฉดในกลุ่มเรียงจากอ่อนไปเข้มเสมอ** ไม่ใช่ตามลำดับที่บังเอิญเพิ่มเข้าตู้ ผลลัพธ์จึงเท่าเดิมทุกครั้ง
