...
- **Widened during implementation (when the tint updates):** this bullet says the tint is derived "on every regenerate", but the shipped app also re-derives it when the user pans to another card in the same batch. Panning to a different outfit and leaving the background on the first one's color contradicts the goal the ux doc states for this feature (G8, "the theme feels alive with the outfit you are looking at"), so the trigger is the focused outfit changing, whatever changed it.
...
- **Renamed during implementation:** the button is not labelled "ดูสีเพิ่มเติม" in the shipped app, because neither entry point adds colors. Entry (a) deliberately shows a *subset* of its row (owned colors only, per the bullet above, while the row also carries presets), and entry (b) shows the *same* palette the row already has, only grouped. The labels are "ดูสีในตู้ตามโทน" and "ดูสีทั้งหมดตามโทน", which say what actually happens.
